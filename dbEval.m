...
for d=1:length(dataNames), dataName=dataNames{d};
    ...
    dts = loadDt( algs, plotName, aspectRatio, saveFiles );
    gts = loadGt( exps, plotName, aspectRatio, bnds );
    res = evalAlgs( plotName, algs, exps, gts, dts, saveFiles );
...
function res = evalAlgs( plotName, algs, exps, gts, dts, saveFiles )
...
for g=1:nGt
    for d=1:nDt
        ...
        res(g,d)=R; if saveFiles, save(fName,'R'); end;
    end
end
end
...
for p=1:nPlots
    ...
    if( plotAlg )
        xs1=xs(:,p); ys1=ys(:,p); fName1=[fName sprintf('_%03d_',p) stra{p}]; lgd1=stre;
...
%         styles1=styles(ord); 
    ...
    if(~isempty(lgd1)), legend(h,lgd1,'Location','sw','FontSize',11, 'Interpreter', 'none'); end
    % save figure to disk (uncomment pdfcrop commands to automatically crop)
    saveas(1,[fName1 '.png']);
    savefig2(fName1,1,'pdf','-r300','-fonts'); %close(1);
...
for page=1:min(pPage,ceil(n/mRows/nCols))
    ...
    savefig2([fName int2str2(page-1,2)],h,'png','-r200','-fonts'); %close(h);
...
function dts = loadDt( algs, plotName, aspectRatio, saveFiles )
